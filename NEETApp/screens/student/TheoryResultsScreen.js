...
import { getTheoryResults } from "../../services/googleSheetsService";
import {
  View, Text, StyleSheet, ScrollView,
  TouchableOpacity
} from 'react-native';

export default function TheoryResultsScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const loadResults = async () => {
      console.log(`[TheoryResultsScreen] Loading theory results for roll number: ${rollNumber}`);
      const data = await getTheoryResults(rollNumber);
      console.log(`[TheoryResultsScreen] Results loaded:`, Object.keys(data).length, 'entries');
      setResults(data);
    };
  ...
  resultDates.forEach((date) => {
    markedDates[date] = { marked: true, dotColor: '#8E44AD' };
  });
  if (selectedDate) {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerLabel}>OJAS THEORY TEST</Text>
        <Text style={styles.headerTitle}>Theory Test Result</Text>
      ...
      <View style={styles.summaryCard}>
        <Text style={styles.summaryTestLabel}>Theory Test {resultDates.length - currentIndex}</Text>
      ...
      <View style={styles.subjectsSection}>
        ...
        {subjects.map((sub) => {
          ...
          const outOf = data.totalMarks;
          const subPct = outOf ? Math.round((marks / outOf) * 100) : 0;
          ...
          return (
            <View key={sub} style={styles.subCard}>
              <View style={styles.subCardHeader}>
                ...
                <Text style={[styles.subMarks, { color }]}>{marks} / {outOf}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  navText: { fontSize: 14, fontWeight: '600', color: '#8E44AD' },
