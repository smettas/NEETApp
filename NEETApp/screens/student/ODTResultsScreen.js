...
export default function ODTResultsScreen({ navigation, route }) {
  const { type, title } = route.params || { type: 'PCB', title: 'ODT Results' };
  // Get roll number from route params or show error
  ...
  if (!rollNumber) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={{marginTop: 10, color: '#666', fontSize: 12}}>Please go back and try again</Text>
      </View>
    );
  }

  const subjects = type === 'PCB'
  ...
  useEffect(() => {
    const loadResults = async () => {
      let data;
      console.log(`[ODTResultsScreen] Loading ${type} results for roll number: ${rollNumber}`);
      if (type === 'PCB') {
        data = await getPCBResults(rollNumber);
      } else {
        data = await getPCMResults(rollNumber);
      }
      console.log(`[ODTResultsScreen] Results loaded:`, Object.keys(data).length, 'entries');
      setResults(data);
    };
    loadResults();
  }, [rollNumber, type]);

  const resultDates = Object.keys(results).sort().reverse();

  useEffect(() => {
    if (resultDates.length > 0 && !selectedDate) {
      setSelectedDate(resultDates[0]);
      setTestNumber(resultDates.length);
    }
  }, [resultDates]);

  const currentResult = results[selectedDate] || results[resultDates[0]] || null;
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerLabel}>OJAS DAILY TEST • {type}</Text>
        <Text style={styles.headerTitle}>Daily Test Result</Text>
      ...
      <View style={styles.summaryCard}>
        <Text style={styles.summaryTestLabel}>Daily Test #{resultDates.length - currentIndex}</Text>
